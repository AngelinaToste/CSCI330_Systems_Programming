...
/*'
 * Purpose: create a dynamic struct array where the name is a pointer.
 * - Perform the following functions:
 *     + SCAN() open the file and return an int for how many lines are in the file
 *     + LOAD() open and rewind the file, populate a dynamic array of structs with the information
 *     + SEARCH() use a provided command line argument to search through the array for a matching name
 *     + FREE() free all of the memory allocated in the program, including the names at each index of the struct array
 *
 */

/* modifications made to allow for first and last name of variable length  // works with file hw4c.txt
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
{// open file/ stream, return integer indicating how many lines are in the file
	...
	*stream = fopen("hw4.data", "r");
	...
	else
	{
        ...
        while ( !feof(*stream)) // count the number of lines in the file
        {
        	junk.name = malloc(sizeof(junk.name));
        	fscanf( *stream, "%s %li\n", junk.name, &junk.number);
        	free(junk.name);
        	numOfContacts++;
        }
	}
